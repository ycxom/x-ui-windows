...
func (s *InboundService) GetInbounds(userId int) ([]*model.Inbound, error) {
	...
	err := db.Model(model.Inbound{}).Preload("ClientStats").Select("*").Where("user_id = ?", userId).Find(&inbounds).Error
...
func (s *InboundService) GetAllInbounds() ([]*model.Inbound, error) {
	db := database.GetDB()
	var inbounds []*model.Inbound
	err := db.Model(model.Inbound{}).Preload("ClientStats").Select("*").Find(&inbounds).Error
	if err != nil && err != gorm.ErrRecordNotFound {
		return nil, err
	}
	return inbounds, nil
